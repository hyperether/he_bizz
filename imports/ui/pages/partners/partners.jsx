...
export class PartnersPage extends Component {
	...
	<div>
		<div className="page-container container" id="content">
			...
			<PartnersPageList data={this.props.data} routeParams={this.props.routeParams} />
		</div>
	</div>
);
		}
...
export const PartnersPageContainer = withTracker(function(props) {
...
export class PartnersPageList extends Component {
	constructor () {
		super();

		this.state = {
			PartnersPageListSearchString: "",
			PartnersPageListSortBy: "",
			PartnersPageListStyle: "table"
		};
	...
	isNotFound() {
		return this.props.data.customers && this.props.data.customers.length == 0 && this.state.PartnersPageListSearchString;
	}

	onInsert(e) {
		FlowRouter.go("partners.insert", objectUtils.mergeObjects(FlowRouter.current().params, {}));
	}

	onSearchInputChange(e) {
		this.setState({PartnersPageListSearchString: e.target.value});
	}
	...
	onSearch(e) {
		...
		this.setState({ PartnersPageListSearchString: searchString });
	}

	onSort(e) {
		e.preventDefault();
		let sortBy = $(e.currentTarget).attr("data-sort");
		this.setState({ PartnersPageListSortBy: sortBy });
	}
	...
	<div id="dataview-data-table">
		<table id="dataview-table" className="table table-striped">
			...
			return(
				<PartnersPageListTableItems key={item._id} data={item} routeParams={self.props.routeParams} onDelete={self.onDelete} parentData={parentData} />
				);
		})}
			</tbody>
		</table>
	...
	renderData() {
		let viewStyle = this.state.PartnersPageListStyle || "table";
		switch(viewStyle) {
	...
	render() {
		return (
	<div id="partners-page-list" className="">
				...
				<form id="dataview-controls" className="form-inline">
					...
					<div id="dataview-controls-search" className="form-group">
						<div id="dataview-controls-search-group" className="input-group">
							<input type="text" className="form-control" id="dataview-search-input" placeholder="Search" name="search" defaultValue={this.state.PartnersPageListSearchString} onChange={this.onSearchInputChange} autoFocus={true} />
		...
		<div className="alert alert-warning">
			{"\"" + this.state.PartnersPageListSearchString + "\" not found."}
		</div>
		:
...
export class PartnersPageListTableItems extends Component {
	constructor() {
		super();
	...
	onEdit(e) {
		...
		let self = this;
		let itemId = this.props.data._id;
		FlowRouter.go("partners.edit", objectUtils.mergeObjects(FlowRouter.current().params, {customerId: this.props.data._id}));
	}
