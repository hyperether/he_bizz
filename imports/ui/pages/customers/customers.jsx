...
export class CustomersPage extends Component {
	...
	<div>
		<div className="page-container container" id="content">
			...
			<CustomersPageList data={this.props.data} routeParams={this.props.routeParams} />
		</div>
	</div>
);
		}
...
export class CustomersPageList extends Component {
	constructor () {
		super();

		this.state = {
			CustomersPageListSearchString: "",
			CustomersPageListSortBy: "",
			CustomersPageListStyle: "table"
		};
	...
	isNotFound() {
		return this.props.data.customers && this.props.data.customers.length == 0 && this.state.CustomersPageListSearchString;
	}
	...
	onSearchInputChange(e) {
		this.setState({CustomersPageListSearchString: e.target.value});
	}
	...
	onSearch(e) {
		...
		this.setState({ CustomersPageListSearchString: searchString });
	}

	onSort(e) {
		e.preventDefault();
		let sortBy = $(e.currentTarget).attr("data-sort");
		this.setState({ CustomersPageListSortBy: sortBy });
	}
	...
	<div id="dataview-data-table">
		<table id="dataview-table" className="table table-striped">
			...
			return(
				<CustomersPageListTableItems key={item._id} data={item} routeParams={self.props.routeParams} onDelete={self.onDelete} parentData={parentData} />
				);
		})}
			</tbody>
		</table>
	...
	renderData() {
		let viewStyle = this.state.CustomersPageListStyle || "table";
		switch(viewStyle) {
	...
	render() {
		return (
	<div id="customers-page-list" className="">
				...
				<form id="dataview-controls" className="form-inline">
					...
					<div id="dataview-controls-search" className="form-group">
						<div id="dataview-controls-search-group" className="input-group">
							<input type="text" className="form-control" id="dataview-search-input" placeholder="Search" name="search" defaultValue={this.state.CustomersPageListSearchString} onChange={this.onSearchInputChange} autoFocus={true} />
		...
		<div className="alert alert-warning">
			{"\"" + this.state.CustomersPageListSearchString + "\" not found."}
		</div>
		:
...
export class CustomersPageListTableItems extends Component {
	constructor() {
		super();
