...
export class PaycheksPage extends Component {
	constructor () {
		super();
	...
	<div>
		<div className="page-container container" id="content">
			...
			<PaycheksPageList data={this.props.data} routeParams={this.props.routeParams} />
		</div>
	</div>
);
		}
...
export const PaycheksPageContainer = withTracker(function(props) {
...
export class PaycheksPageList extends Component {
	constructor () {
		super();

		this.state = {
			PaycheksPageListSearchString: "",
			PaycheksPageListSortBy: "",
			PaycheksPageListStyle: "table"
		};
	...
	isNotFound() {
		return this.props.data.customers && this.props.data.customers.length == 0 && this.state.PaycheksPageListSearchString;
	}

	onInsert(e) {
		FlowRouter.go("paycheks.insert", objectUtils.mergeObjects(FlowRouter.current().params, {}));
	}

	onSearchInputChange(e) {
		this.setState({PaycheksPageListSearchString: e.target.value});
	}
	...
	onSearch(e) {
		...
		this.setState({ PaycheksPageListSearchString: searchString });
	}

	onSort(e) {
		e.preventDefault();
		let sortBy = $(e.currentTarget).attr("data-sort");
		this.setState({ PaycheksPageListSortBy: sortBy });
	}
	...
	<div id="dataview-data-table">
		<table id="dataview-table" className="table table-striped">
			...
			return(
				<PaycheksPageListTableItems key={item._id} data={item} routeParams={self.props.routeParams} onDelete={self.onDelete} parentData={parentData} />
				);
		})}
			</tbody>
		</table>
	...
	renderData() {
		let viewStyle = this.state.PaycheksPageListStyle || "table";
		switch(viewStyle) {
	...
	render() {
		return (
	<div id="paycheks-page-list" className="">
				...
				<form id="dataview-controls" className="form-inline">
					...
					<div id="dataview-controls-search" className="form-group">
						<div id="dataview-controls-search-group" className="input-group">
							<input type="text" className="form-control" id="dataview-search-input" placeholder="Search" name="search" defaultValue={this.state.PaycheksPageListSearchString} onChange={this.onSearchInputChange} autoFocus={true} />
		...
		<div className="alert alert-warning">
			{"\"" + this.state.PaycheksPageListSearchString + "\" not found."}
		</div>
		:
...
export class PaycheksPageListTableItems extends Component {
	constructor() {
		super();
	...
	onEdit(e) {
		...
		let self = this;
		let itemId = this.props.data._id;
		FlowRouter.go("paycheks.edit", objectUtils.mergeObjects(FlowRouter.current().params, {customerId: this.props.data._id}));
	}
