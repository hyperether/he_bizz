...
export class ProjectsPage extends Component {
	...
	<div>
		<div className="page-container container" id="content">
			...
			<ProjectsPageList data={this.props.data} routeParams={this.props.routeParams} />
		</div>
	</div>
);
		}
...
export const ProjectsPageContainer = withTracker(function(props) {
...
export class ProjectsPageList extends Component {
	constructor () {
		super();

		this.state = {
			ProjectsPageListSearchString: "",
			ProjectsPageListSortBy: "",
			ProjectsPageListStyle: "table"
		};
	...
	isNotFound() {
		return this.props.data.customers && this.props.data.customers.length == 0 && this.state.ProjectsPageListSearchString;
	}

	onInsert(e) {
		FlowRouter.go("projects.insert", objectUtils.mergeObjects(FlowRouter.current().params, {}));
	}

	onSearchInputChange(e) {
		this.setState({ProjectsPageListSearchString: e.target.value});
	}
	...
	onSearch(e) {
		...
		this.setState({ ProjectsPageListSearchString: searchString });
	}

	onSort(e) {
		e.preventDefault();
		let sortBy = $(e.currentTarget).attr("data-sort");
		this.setState({ ProjectsPageListSortBy: sortBy });
	}
	...
	<div id="dataview-data-table">
		<table id="dataview-table" className="table table-striped">
			...
			return(
				<ProjectsPageListTableItems key={item._id} data={item} routeParams={self.props.routeParams} onDelete={self.onDelete} parentData={parentData} />
				);
		})}
			</tbody>
		</table>
	...
	renderData() {
		let viewStyle = this.state.ProjectsPageListStyle || "table";
		switch(viewStyle) {
	...
	render() {
		return (
	<div id="projects-page-list" className="">
				...
				<form id="dataview-controls" className="form-inline">
					...
					<div id="dataview-controls-search" className="form-group">
						<div id="dataview-controls-search-group" className="input-group">
							<input type="text" className="form-control" id="dataview-search-input" placeholder="Search" name="search" defaultValue={this.state.ProjectsPageListSearchString} onChange={this.onSearchInputChange} autoFocus={true} />
		...
		<div className="alert alert-warning">
			{"\"" + this.state.ProjectsPageListSearchString + "\" not found."}
		</div>
		:
...
export class ProjectsPageListTableItems extends Component {
	constructor() {
		super();
	...
	onEdit(e) {
		...
		let self = this;
		let itemId = this.props.data._id;
		FlowRouter.go("projects.edit", objectUtils.mergeObjects(FlowRouter.current().params, {customerId: this.props.data._id}));
	}
