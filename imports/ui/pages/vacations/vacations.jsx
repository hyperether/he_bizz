...
export class VacationsPage extends Component {
	constructor () {
		super();
	...
	<div>
		<div className="page-container container" id="content">
			...
			<VacationsPageList data={this.props.data} routeParams={this.props.routeParams} />
		</div>
	</div>
);
		}
...
export const VacationsPageContainer = withTracker(function(props) {
...
export class VacationsPageList extends Component {
	constructor () {
		super();

		this.state = {
			VacationsPageListSearchString: "",
			VacationsPageListSortBy: "",
			VacationsPageListStyle: "table"
		};
	...
	isNotFound() {
		return this.props.data.customers && this.props.data.customers.length == 0 && this.state.VacationsPageListSearchString;
	}

	onInsert(e) {
		FlowRouter.go("vacations.insert", objectUtils.mergeObjects(FlowRouter.current().params, {}));
	}

	onSearchInputChange(e) {
		this.setState({VacationsPageListSearchString: e.target.value});
	}
	...
	onSearch(e) {
		...
		this.setState({ VacationsPageListSearchString: searchString });
	}

	onSort(e) {
		e.preventDefault();
		let sortBy = $(e.currentTarget).attr("data-sort");
		this.setState({ VacationsPageListSortBy: sortBy });
	}
	...
	<div id="dataview-data-table">
		<table id="dataview-table" className="table table-striped">
			...
			return(
				<VacationsPageListTableItems key={item._id} data={item} routeParams={self.props.routeParams} onDelete={self.onDelete} parentData={parentData} />
				);
		})}
			</tbody>
		</table>
	...
	renderData() {
		let viewStyle = this.state.VacationsPageListStyle || "table";
		switch(viewStyle) {
	...
	render() {
		return (
	<div id="vacations-page-list" className="">
				...
				<form id="dataview-controls" className="form-inline">
					...
					<div id="dataview-controls-search" className="form-group">
						<div id="dataview-controls-search-group" className="input-group">
							<input type="text" className="form-control" id="dataview-search-input" placeholder="Search" name="search" defaultValue={this.state.VacationsPageListSearchString} onChange={this.onSearchInputChange} autoFocus={true} />
		...
		<div className="alert alert-warning">
			{"\"" + this.state.VacationsPageListSearchString + "\" not found."}
		</div>
		:
...
export class VacationsPageListTableItems extends Component {
	constructor() {
		super();
	...
	onEdit(e) {
		...
		let self = this;
		let itemId = this.props.data._id;
		FlowRouter.go("vacations.edit", objectUtils.mergeObjects(FlowRouter.current().params, {customerId: this.props.data._id}));
	}
