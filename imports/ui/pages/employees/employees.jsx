...
export class EmployeesPage extends Component {
	...
	<div>
		<div className="page-container container" id="content">
			...
			<EmployeesPageList data={this.props.data} routeParams={this.props.routeParams} />
		</div>
	</div>
);
		}
...
export const EmployeesPageContainer = withTracker(function(props) {
...
export class EmployeesPageList extends Component {
	constructor () {
		super();

		this.state = {
			EmployeesPageListSearchString: "",
			EmployeesPageListSortBy: "",
			EmployeesPageListStyle: "table"
		};
	...
	isNotFound() {
		return this.props.data.customers && this.props.data.customers.length == 0 && this.state.EmployeesPageListSearchString;
	}

	onInsert(e) {
		FlowRouter.go("employees.insert", objectUtils.mergeObjects(FlowRouter.current().params, {}));
	}

	onSearchInputChange(e) {
		this.setState({EmployeesPageListSearchString: e.target.value});
	}
	...
	onSearch(e) {
		...
		this.setState({ EmployeesPageListSearchString: searchString });
	}

	onSort(e) {
		e.preventDefault();
		let sortBy = $(e.currentTarget).attr("data-sort");
		this.setState({ EmployeesPageListSortBy: sortBy });
	}
	...
	<div id="dataview-data-table">
		<table id="dataview-table" className="table table-striped">
			...
			return(
				<EmployeesPageListTableItems key={item._id} data={item} routeParams={self.props.routeParams} onDelete={self.onDelete} parentData={parentData} />
				);
		})}
			</tbody>
		</table>
	...
	renderData() {
		let viewStyle = this.state.EmployeesPageListStyle || "table";
		switch(viewStyle) {
	...
	render() {
		return (
	<div id="employees-page-list" className="">
				...
				<form id="dataview-controls" className="form-inline">
					...
					<div id="dataview-controls-search" className="form-group">
						<div id="dataview-controls-search-group" className="input-group">
							<input type="text" className="form-control" id="dataview-search-input" placeholder="Search" name="search" defaultValue={this.state.EmployeesPageListSearchString} onChange={this.onSearchInputChange} autoFocus={true} />
		...
		<div className="alert alert-warning">
			{"\"" + this.state.EmployeesPageListSearchString + "\" not found."}
		</div>
		:
...
export class EmployeesPageListTableItems extends Component {
	constructor() {
		super();
	...
	onEdit(e) {
		...
		let self = this;
		let itemId = this.props.data._id;
		FlowRouter.go("employees.edit", objectUtils.mergeObjects(FlowRouter.current().params, {customerId: this.props.data._id}));
	}
