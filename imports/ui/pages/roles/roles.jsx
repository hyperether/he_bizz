...
export class RolesPage extends Component {
	...
	<div>
		<div className="page-container container" id="content">
			...
			<RolesPageList data={this.props.data} routeParams={this.props.routeParams} />
		</div>
	</div>
);
		}
...
export const RolesPageContainer = withTracker(function(props) {
...
export class RolesPageList extends Component {
	constructor () {
		super();

		this.state = {
			RolesPageListSearchString: "",
			RolesPageListSortBy: "",
			RolesPageListStyle: "table"
		};
	...
	isNotFound() {
		return this.props.data.customers && this.props.data.customers.length == 0 && this.state.RolesPageListSearchString;
	}

	onInsert(e) {
		FlowRouter.go("roles.insert", objectUtils.mergeObjects(FlowRouter.current().params, {}));
	}

	onSearchInputChange(e) {
		this.setState({RolesPageListSearchString: e.target.value});
	}
	...
	onSearch(e) {
		...
		this.setState({ RolesPageListSearchString: searchString });
	}

	onSort(e) {
		e.preventDefault();
		let sortBy = $(e.currentTarget).attr("data-sort");
		this.setState({ RolesPageListSortBy: sortBy });
	}
	...
	<div id="dataview-data-table">
		<table id="dataview-table" className="table table-striped">
			...
			return(
				<RolesPageListTableItems key={item._id} data={item} routeParams={self.props.routeParams} onDelete={self.onDelete} parentData={parentData} />
				);
		})}
			</tbody>
		</table>
	...
	renderData() {
		let viewStyle = this.state.RolesPageListStyle || "table";
		switch(viewStyle) {
	...
	render() {
		return (
	<div id="roles-page-list" className="">
				...
				<form id="dataview-controls" className="form-inline">
					...
					<div id="dataview-controls-search" className="form-group">
						<div id="dataview-controls-search-group" className="input-group">
							<input type="text" className="form-control" id="dataview-search-input" placeholder="Search" name="search" defaultValue={this.state.RolesPageListSearchString} onChange={this.onSearchInputChange} autoFocus={true} />
		...
		<div className="alert alert-warning">
			{"\"" + this.state.RolesPageListSearchString + "\" not found."}
		</div>
		:
...
export class RolesPageListTableItems extends Component {
	constructor() {
		super();
	...
	onEdit(e) {
		...
		let self = this;
		let itemId = this.props.data._id;
		FlowRouter.go("roles.edit", objectUtils.mergeObjects(FlowRouter.current().params, {customerId: this.props.data._id}));
	}
